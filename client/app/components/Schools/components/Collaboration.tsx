...
const Collaboration = () => {
  ...
  return (
    <Section crosses className="py-12 md:py-16 lg:py-20">
      <div className="container flex flex-col lg:flex-row gap-8 md:gap-12 lg:gap-16 px-4 sm:px-6">
        {/* Left Column - Content */}
        <div className="w-full lg:max-w-[25rem]">
          ...
          <ul className="max-w-full md:max-w-[22rem] mb-8 md:mb-14 mx-auto lg:mx-0">
            {schools.collabItems.map((item, index) => (
              <li 
                key={index} 
                className="mb-3 py-3 border-b border-n-6 last:border-0"
              >
                <Accordion 
                  open={open === index + 1}
        ...
        <div className="w-full lg:ml-auto xl:w-[38rem] mt-0 lg:mt-4">
          ...
          <div className="relative mx-auto w-full max-w-[18rem] sm:max-w-[22rem] aspect-square border border-n-6 rounded-full">
            <div className="absolute inset-0 flex items-center justify-center w-full h-full">
              <div className="flex w-48 sm:w-60 aspect-square m-auto border border-n-6 rounded-full">
                <div className="w-[5rem] sm:w-[6rem] aspect-square m-auto p-1 sm:p-[0.2rem] bg-conic-gradient rounded-full">
            ...
            <ul className="absolute inset-0">
              {collabApps.map((app, index) => (
                <li
                  key={app.id}
                  className={`absolute top-0 left-1/2 h-1/2 -ml-[1.2rem] sm:-ml-[1.6rem] origin-bottom rotate-${
                    index * 45
                  }`}
                >
                  <div
                    className={`relative -top-[1.2rem] sm:-top-[1.6rem] flex w-[2.8rem] sm:w-[3.2rem] h-[2.8rem] sm:h-[3.2rem] bg-n-7 border border-n-1/15 rounded-xl -rotate-${
                      index * 45
                    }`}
                  >
                    <Image
